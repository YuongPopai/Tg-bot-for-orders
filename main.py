...
from config import base_url, secret_key, headers, sbpMerchantId, user_file_path, admin_file_path, \
    bot_key  # type: ignore
import pandas as pd
import uuid
...
def create_qr_code(message):
    ...
    def step(message):
        if re.match(r'\d*', message.text):
            dict = rf.create_qr_request(base_url,
                                        {"qrType": "QRDynamic", "amount": message.text, "order": generate_order(),
...
def admin_btns_init(message) -> None:
    ...
    btn6 = types.KeyboardButton("🛠удалить qr-code")
...
def main(message):
    ...
    global is_admin, is_user, is_waiting_for_password
    if message.text == '🆔Получить статус по id':
        get_qr_status_by_id(message)
        return
    elif not is_admin:
        bot.send_message(message.chat.id,
                         text="Вы ошиблись в запросе или пока не обладаете правом использование данной функцией")
        return
        ...
        get_qr_code(message)
        return
    elif message.text == '🛠удалить qr-code':
